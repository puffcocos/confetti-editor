...
**Figma, Illustrator, Inkscape** 등에서 도형을 만들고 SVG로 내보낸 후 `<path>` 요소의 `d` 속성 값을 추출합니다.
...
**Font Awesome**, **Material Icons** 등의 아이콘 SVG에서 path를 추출할 수 있습니다.

```tsx
// Material Icons의 별 아이콘
...
미리보기 페이지에서 커스텀 파티클을 프리셋으로 저장할 수 있습니다.

### 웹 UI에서 저장

1. `/preview` 페이지 접속
2. "커스텀 파티클" 섹션에서 SVG Path 입력
3. 프리셋 이름 입력
4. "저장" 버튼 클릭
...
function MyComponent() {
  const [shapePresets, setShapePresets] = useLocalStorage<CustomShapePreset[]>(
    'confetti-custom-shape-presets',
    []
  )

  const saveShape = (name: string, path: string) => {
    setShapePresets([...shapePresets, { name, path }])
  }

  return <button onClick={() => saveShape('my-heart', 'M5 2 C5 0.5...')}>하트 저장</button>
}
```
...
SVG Path를 미리 볼 수 있는 컴포넌트를 제공합니다.

```tsx
import { SvgPathPreview } from '~/components/svg-path-preview'

function MyComponent() {
  const path = 'M5 2 C5 0.5 6 0 7 0...'

  return <SvgPathPreview path={path} width={100} height={100} className="border border-gray-300" />
}
```

이 컴포넌트는 SVG path의 bounding box를 자동으로 계산하여 적절한 viewBox로 렌더링합니다.
...
- [트러블슈팅](./troubleshooting.md) - 커스텀 파티클 문제 해결
